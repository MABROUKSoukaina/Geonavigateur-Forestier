...
import { EXPOSITION, TOPO_POS, SUBSTRAT, TYPE_SOL, PROFONDEUR_SOL, HUMUS, AFFLEUREMENT_ROCHEUX, CAILLOUX } from '../labels';
      ...
      {detail && !loading && (
        <>
          <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))' }}>
            <Panel title="Structure en circonférences C 1,3">
              <HistogramChart bins={detail.structure_circonference} unit="Circonférence C1.3 (cm)" color={T.cyan} />
            </Panel>
            <Panel title="Structure en circonférences C 0,2">
              <HistogramChart bins={detail.structure_c0} unit="Circonférence C 0,2 (cm)" color={T.violet} />
            </Panel>
            ...
            <Panel title="Profondeur du sol des peuplements">
              <CategoricalBarChart data={decode(detail.profondeur_sol, PROFONDEUR_SOL)} color={T.violet} />
            </Panel>
            <Panel title="Humus des sols des peuplements">
              <CategoricalBarChart data={decode(detail.humus, HUMUS)} color={T.greenLite} />
            </Panel>
            <Panel title="Affleurement rocheux">
              <CategoricalBarChart data={decode(detail.affleurement_rocheux, AFFLEUREMENT_ROCHEUX)} color={T.red} />
            </Panel>
            <Panel title="Cailloux dans les peuplements">
              <CategoricalBarChart data={decode(detail.cailloux, CAILLOUX)} color={T.cyan} />
            </Panel>
          </div>
        </>
      )}
