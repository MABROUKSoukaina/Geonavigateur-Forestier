...
export const INDICATEURS = [
  ...
  { value: 'circonference_moyenne', label: 'Circonférence 1,3 moyenne (cm)', short: 'Circonférence 1,3', unit: 'cm',   decimals: 1 },
...
export function AnalyseForestiere({ rows, loading, analyse, ecosystemes }: {
  ...
  return (
    <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'minmax(560px, 1fr) 280px' }}>
      {/* ── Detailed table ─────────────────────────────────────────── */}
      <Panel
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, minWidth: 620 }}>
              <thead>
                <tr style={{ background: T.panelAlt }}>
                  ...
                  <th style={th}>Arbres inventoriés</th>
                  <th style={th}>Arbres sur pied</th>
                  <th style={th}>Arbres coupés</th>
                  <th style={th}>Arbres morts</th>
                  {metricCols.map(c => (
              ...
              <tbody>
                {tableRows.map((r, i) => (
                  <tr key={i}>
                    ...
                    <td style={td}>{fmt(r.nb_arbres)}</td>
                    <td style={td}>{fmt(r.nb_vivants)}</td>
                    <td style={td}>{fmt(r.nb_coupes)}</td>
                    <td style={td}>{fmt(r.nb_morts)}</td>
                    {metricCols.map(c => {
                      const v = r[c.value];
                ...
                {tableRows.length === 0 && (
                  <tr><td colSpan={metricCols.length + 6} style={{ ...td, color: T.dim, padding: '18px 0' }}>
