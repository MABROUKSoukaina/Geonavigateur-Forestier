...
const EQUIPE_COLORS = ['#a5b4fc', '#86efac', '#fb923c', '#f472b6', '#7dd3fc', '#d8b4fe'];
...
function makeMarkerIcon(color: string, highlighted: boolean, zoom: number): L.DivIcon {
  ...
  return L.divIcon({
    className: '',
    html: `<div style="width:${hit}px;height:${hit}px;display:flex;align-items:center;justify-content:center;"><div style="width:${dot}px;height:${dot}px;background:${color};border:1px solid rgba(255,255,255,0.85);border-radius:50%;box-shadow:${glow}0 2px 6px rgba(0,0,0,0.3);box-sizing:border-box;"></div></div>`,
...
function markerColor(f: MapFeature, mode: ClassifyMode, essenceColors: Map<string, string>): string {
  ...
  if (mode === 'accessibilite') {
    if (f.properties.accessibilite === 1) return '#22d3ee';
    if (f.properties.accessibilite === 0) return '#f87171';
    return '#fb923c';
  }
  if (mode === 'controle') {
    if (f.properties.statut === 'controle')   return '#d946ef';  // contrôlée — red
    if (f.properties.statut === 'visitee')    return '#34d399';  // non contrôlée — green
    return '#fb923c';                                             // programmée — orange
  }
  if (f.properties.statut === 'visitee' || f.properties.statut === 'controle') return '#34d399';
  if (f.properties.statut === 'programmee') return '#fb923c';
  return '#fb923c';
}
...
const miniBtn: CSSProperties = {
  ...
  padding: '2px 9px', cursor: 'pointer', fontSize: 10, color: '#bac4d0',
};
...
function FilterBtn({ label, active, badge, onClick }: {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      background: active ? 'rgba(5,150,105,0.15)' : '#1a2540',
      border: `1px solid ${active ? 'rgba(5,150,105,0.5)' : 'rgba(255,255,255,0.1)'}`,
      borderRadius: 8, padding: '5px 10px', cursor: 'pointer',
      fontSize: 12, fontWeight: 500,
      color: active ? '#6ee7b7' : '#bac4d0',
...
function DropPanel({ children, style }: { children: ReactNode; style?: CSSProperties }) {
  return (
    <div style={{
      ...
      boxShadow: '0 8px 32px rgba(0,0,0,0.4)', padding: '12px 14px',
      minWidth: 240, ...style,
    }}>
      {children}
    </div>
...
export function TabCarte({ data }: Props) {
  ...
  const [selControle,  setSelControle]  = useState<Set<string>>(new Set());
  const [selAccessibilite, setSelAccessibilite] = useState<Set<string>>(new Set());
  const [openPanel,    setOpenPanel]    = useState<'classifier' | 'statuts' | 'controle' | 'equipes' | 'essences' | 'accessibilite' | null>(null);
  ...
  // Re-fetch map whenever dashboard data changes (new upload, service restart, daily refresh)
  useEffect(() => {
    fetchMapGeoJson()
      .then(d => setAllFeatures(d.features))
      .catch(e => setError(e instanceof Error ? e.message : 'Erreur réseau'))
      .finally(() => setLoading(false));
  }, [data.kpi.total_visitees, data.kpi.nb_controle, data.kpi.nb_controle_service]);
  ...
  const filtered = allFeatures
    .filter(f => {
      if (selStatuts.size === 0) return true;
      const isRealisee = f.properties.statut === 'visitee' || f.properties.statut === 'controle';
      if (selStatuts.has('realisee') && isRealisee) return true;
      if (selStatuts.has('programmee') && f.properties.statut === 'programmee') return true;
      return false;
    })
    .filter(f => {
      if (selControle.size === 0) return true;
      if (selControle.has('controle')   && f.properties.statut === 'controle')   return true;
      if (selControle.has('visitee')    && f.properties.statut === 'visitee')    return true;
      if (selControle.has('programmee') && f.properties.statut === 'programmee') return true;
      return false;
    })
  ...
  const hasFilters = selStatuts.size > 0 || selControle.size > 0 || selEquipes.size > 0 || selEssences.size > 0 || selAccessibilite.size > 0;
  ...
  function togglePanel(p: 'classifier' | 'statuts' | 'controle' | 'equipes' | 'essences' | 'accessibilite'): void {
  ...
  const getIconFn = (f: MapFeature): L.DivIcon => {
    ...
    return makeMarkerIcon(highlighted ? '#fb923c' : color, highlighted, zoom);
  };
  ...
  const buildPopupFn = (f: MapFeature): ReactNode => {
    ...
    return (
      <div style={{ fontFamily: 'system-ui, sans-serif' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 9, paddingBottom: 8, borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ width: 10, height: 10, borderRadius: '50%', background: color, flexShrink: 0 }} />
          <span style={{ fontWeight: 700, fontSize: 14, color: '#edf1f5' }}>{f.properties.num_placette}</span>
          {isDone && (
            <span style={{ marginLeft: 'auto', marginRight: 20, fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 20, background: isControle ? 'rgba(139,92,246,0.12)' : 'rgba(16,185,129,0.12)', color: isControle ? '#8b5cf6' : '#10b981' }}>
              {badgeLabel}
            </span>
          )}
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '5px 14px', fontSize: 12 }}>
          {f.properties.equipe && <><span style={{ color: '#94a3b8' }}>Équipe</span><span style={{ color: '#edf1f5', fontWeight: 600 }}>{equipeShort(f.properties.equipe)}</span></>}
          {f.properties.essence_group && <><span style={{ color: '#94a3b8' }}>Formation</span><span style={{ color: '#edf1f5', fontWeight: 600 }}>{f.properties.essence_group}</span></>}
          {f.properties.dpanef && <><span style={{ color: '#94a3b8' }}>DPANEF</span><span style={{ color: '#edf1f5', fontWeight: 600 }}>{f.properties.dpanef}</span></>}
          {f.properties.altitude != null && <><span style={{ color: '#94a3b8' }}>Altitude</span><span style={{ color: '#edf1f5', fontWeight: 600 }}>{f.properties.altitude} m</span></>}
          {f.properties.pente != null && <><span style={{ color: '#94a3b8' }}>Pente</span><span style={{ color: '#edf1f5', fontWeight: 600 }}>{f.properties.pente}°</span></>}
          {f.properties.distance_repere != null && <><span style={{ color: '#94a3b8' }}>Dist. repère</span><span style={{ color: '#edf1f5', fontWeight: 600 }}>{f.properties.distance_repere} m</span></>}
        </div>
        {f.properties.description_repere && (
          <div style={{ marginTop: 8, paddingTop: 7, fontSize: 10.5, color: '#7a8a9c', borderTop: '1px solid rgba(255,255,255,0.08)', lineHeight: 1.45 }}>
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4, minHeight: 0 }}>
      ...
      <div ref={toolbarRef} style={{
        ...
        background: '#162035', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10,
        ...
        <div style={{ position: 'relative' }}>
          ...
          {openPanel === 'classifier' && (
            <DropPanel>
              ...
              {(['statut', 'equipe', 'essence', 'accessibilite', 'controle'] as ClassifyMode[]).map(m => (
                <label key={m} style={{
                  display: 'flex', alignItems: 'center', gap: 8,
                  padding: '5px 0', cursor: 'pointer', fontSize: 12, color: '#dde4ec',
                }}>
                  <input
            ...
            return (
              <DropPanel style={{ minWidth: 220 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                  ...
                  <button style={miniBtn} onClick={() => setSelStatuts(selStatuts.size > 0 ? new Set() : new Set(['realisee', 'programmee']))}>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 0', cursor: 'pointer' }}>
                  <Checkbox
                    size="small"
                    checked={selStatuts.has('realisee')}
                    onChange={() => setSelStatuts(prev => { const n = new Set(prev); if (n.has('realisee')) n.delete('realisee'); else n.add('realisee'); return n; })}
                  ...
                  <span style={{ fontSize: 12, fontWeight: 600, color: '#dde4ec', flex: 1 }}>Placettes réalisées</span>
                  <span style={{ fontSize: 10, fontFamily: 'monospace', color: '#7a8a9c' }}>{cntRealisee}</span>
                </label>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 0', cursor: 'pointer' }}>
                  <Checkbox
                    ...
                    onChange={() => setSelStatuts(prev => { const n = new Set(prev); if (n.has('programmee')) n.delete('programmee'); else n.add('programmee'); return n; })}
              ...
              {equipeList.map(eq => {
                ...
                return (
                  <label key={eq} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '3px 0', cursor: 'pointer' }}>
                    ...
                    <span style={{ fontSize: 12, color: '#dde4ec', flex: 1 }}>{equipeShort(eq)}</span>
                    <span style={{ fontSize: 10, fontFamily: 'monospace', color: count > 0 ? '#7a8a9c' : '#cbd5e1' }}>{count}</span>
                  </label>
                );
              })}
              ...
              {essenceList.map(s => {
                ...
                return (
                  <label key={s} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '3px 0', cursor: 'pointer' }}>
                    ...
                    />
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: color, display: 'inline-block', flexShrink: 0 }} />
                    <span style={{ fontSize: 12, color: '#dde4ec', flex: 1 }}>{s}</span>
                    <span style={{ fontSize: 10, fontFamily: 'monospace', color: count > 0 ? '#7a8a9c' : '#cbd5e1' }}>{count}</span>
                  </label>
                );
              })}
        ...
        <div style={{ position: 'relative' }}>
          ...
          {openPanel === 'accessibilite' && (
            <DropPanel>
              ...
              {[
                { key: '1',    label: 'Placettes accessibles',      color: '#22d3ee' },
                { key: '0',    label: 'Placettes non accessibles',  color: '#f87171' },
                { key: 'null', label: 'Placettes en cours',   color: '#fb923c' },
              ].map(({ key, label, color }) => {
                ...
                return (
                  <label key={key} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '3px 0', cursor: 'pointer' }}>
                    <Checkbox
                      ...
                      onChange={() => setSelAccessibilite(prev => { const n = new Set(prev); if (n.has(key)) { n.delete(key); } else { n.add(key); } return n; })}
                      sx={{ p: 0.3, color: '#cbd5e1', '&.Mui-checked': { color } }}
                    />
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: color, display: 'inline-block', flexShrink: 0 }} />
                    <span style={{ fontSize: 12, color: '#dde4ec', flex: 1 }}>{label}</span>
                    <span style={{ fontSize: 10, fontFamily: 'monospace', color: count > 0 ? '#7a8a9c' : '#cbd5e1' }}>{count}</span>
                  </label>
                );
              })}
        ...
        <div style={{ position: 'relative' }}>
          <FilterBtn
            label="Contrôle"
            active={openPanel === 'controle' || selControle.size > 0}
            badge={selControle.size || undefined}
            onClick={() => togglePanel('controle')}
          />
          {openPanel === 'controle' && (() => {
              ...
              <DropPanel style={{ minWidth: 220 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                  ...
                  <button style={miniBtn} onClick={() => setSelControle(selControle.size > 0 ? new Set() : new Set(['controle', 'visitee', 'programmee']))}>
                    {selControle.size > 0 ? 'Effacer' : 'Tout'}
                  </button>
                </div>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 0', cursor: 'pointer' }}>
                  <Checkbox
                    size="small"
                    checked={selControle.has('programmee')}
                    onChange={() => setSelControle(prev => { const n = new Set(prev); if (n.has('programmee')) n.delete('programmee'); else n.add('programmee'); return n; })}
                    sx={{ p: 0.3, color: '#cbd5e1', '&.Mui-checked': { color: '#fb923c' } }}
                  />
                  <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#fb923c', display: 'inline-block', flexShrink: 0 }} />
                  <span style={{ fontSize: 12, fontWeight: 600, color: '#dde4ec', flex: 1 }}>Placettes en cours</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          ...
          {/* Count chip */}
          <div style={{
            background: '#0b1629', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20,
            height: 26, padding: '0 10px', display: 'flex', alignItems: 'center',
          }}>
            <span style={{ fontSize: 11, color: '#bac4d0', whiteSpace: 'nowrap' }}>
          ...
          {hasFilters && (
            <button
              ...
              style={{
                ...
                background: '#1a2540', border: '1px solid rgba(255,255,255,0.1)', cursor: 'pointer', padding: 0,
              }}
            >
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#7a8a9c" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div style={{
        ...
        {!loading && (
          <div style={{
            ...
            background: 'rgba(22,32,53,0.97)', backdropFilter: 'blur(6px)',
            ...
            <div
              ...
              style={{
                ...
                background: legendOpen ? '#1a2540' : 'transparent',
              }}
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#bac4d0" strokeWidth="2" style={{ flexShrink: 0 }}>
                <line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/>
                <circle cx="3" cy="6" r="1.5" fill="#bac4d0" stroke="none"/><circle cx="3" cy="12" r="1.5" fill="#bac4d0" stroke="none"/><circle cx="3" cy="18" r="1.5" fill="#bac4d0" stroke="none"/>
              </svg>
              {legendOpen && (
                <span style={{ fontSize: 11, fontWeight: 700, color: '#bac4d0', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  Légende
                </span>
              )}
              {legendOpen && (
              ...
              <div style={{ padding: '8px 11px 10px' }}>
                  ...
                  <>
                    {[
                      ...
                      <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 5 }}>
                        ...
                        <span style={{ fontSize: 11, color: '#dde4ec' }}>{label}</span>
                      </div>
                    ))}
                  </>
                )}
                ...
                {classifyMode === 'equipe' && equipeList.map(eq => (
                  <div key={eq} style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 5 }}>
                    ...
                    <span style={{ fontSize: 11, color: '#dde4ec' }}>{equipeShort(eq)}</span>
                  </div>
                ))}
                ...
                {classifyMode === 'essence' && essenceList.map(s => (
                  <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 5 }}>
                    ...
                    <span style={{ fontSize: 11, color: '#dde4ec' }}>{s}</span>
                  </div>
                ))}
                ...
                {classifyMode === 'accessibilite' && (
                  <>
                    {[
                      { color: '#22d3ee', label: 'Placettes accessibles' },
                      { color: '#f87171', label: 'Placettes non accessibles' },
                      { color: '#fb923c', label: 'Placettes en cours' },
                    ].map(({ color, label }) => (
                      <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 5 }}>
                        <span style={{ width: 9, height: 9, borderRadius: '50%', background: color, flexShrink: 0, display: 'inline-block' }} />
        ...
        {!loading && (
          <div ref={searchRef} style={{
            ...
            <div style={{
              ...
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#7a8a9c" strokeWidth="2.5">
                ...
                <>
                  <input
                    ...
                    style={{
                      ...
                      fontSize: 11, color: '#dde4ec', width: '100%',
                      fontFamily: 'inherit',
                    }}
                  />
                  {searchInput && (
            ...
            {searchOpen && (
              <div style={{
                marginTop: 4, background: 'rgba(22,32,53,0.98)', backdropFilter: 'blur(6px)',
                border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10,
                ...
                {searchResults.length === 0 ? (
                  ...
                  const statut = f.properties.statut;
                  const dotColor = statut === 'visitee' ? '#34d399' : statut === 'controle' ? '#d946ef' : '#fb923c';
                  return (
                    <button key={f.properties.num_placette}
                      ...
                      <span style={{ fontSize: 12, fontWeight: isSelected ? 700 : 500, color: '#edf1f5', flex: 1 }}>
        ...
        {loading && (
          <div style={{
            ...
            background: 'rgba(13,27,42,0.85)', backdropFilter: 'blur(2px)',
          }}>
            <div style={{
            ...
            <p style={{ color: '#7a8a9c', fontSize: 13, marginTop: 10 }}>Chargement de la carte…</p>
          </div>
        )}
        ...
        {error && !loading && (
          <div style={{
            ...
            <p style={{ color: '#d946ef', fontSize: 13 }}>Erreur : {error}</p>
          </div>
        )}
        ...
        <div style={{ position: 'absolute', bottom: 28, left: 10, zIndex: 1000 }}>
          {basemapOpen && (
            <div style={{
              marginBottom: 6, background: 'rgba(22,32,53,0.97)',
              backdropFilter: 'blur(6px)', borderRadius: 10,
              border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 4px 16px rgba(0,0,0,0.4)',
              overflow: 'hidden', minWidth: 160,
            }}>
              <div style={{ padding: '7px 12px 5px', fontSize: 10, fontWeight: 700, color: '#7a8a9c', textTransform: 'uppercase', letterSpacing: '0.06em', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                Fond de carte
              </div>
              {BASEMAPS.map(b => (
                <button key={b.id} onClick={() => { setBasemap(b.id); setBasemapOpen(false); }} style={{
                  ...
                  background: basemap === b.id ? 'rgba(16,185,129,0.15)' : 'transparent',
                  color:      basemap === b.id ? '#6ee7b7' : '#dde4ec',
          ...
          <button onClick={() => setBasemapOpen(o => !o)} title="Fond de carte" style={{
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            width: 32, height: 32, border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8,
            background: 'rgba(22,32,53,0.97)', backdropFilter: 'blur(6px)',
            boxShadow: '0 2px 8px rgba(0,0,0,0.1)', cursor: 'pointer',
          }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#bac4d0" strokeWidth="2">
        ...
        >
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#bac4d0" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/>
            <path d="M3 3v5h5"/>
          </svg>
        </button>
