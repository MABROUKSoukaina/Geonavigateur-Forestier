...
public class FormationController {
    ...
    private static final String LOC_CTE =
        ...
        "    COALESCE(reg.description_pedologique_humus, ctrl.description_pedologique_humus, cs.description_pedologique_humus) AS humus, " +
        "    COALESCE(reg.description_pedologique_p_affleurement_rocheux, ctrl.description_pedologique_p_affleurement_rocheux, cs.description_pedologique_p_affleurement_rocheux) AS affleurement_rocheux, " +
        "    COALESCE(reg.description_pedologique_p_cailloux, ctrl.description_pedologique_p_cailloux, cs.description_pedologique_p_cailloux) AS cailloux " +
    ...
    private static final String FORMATION_SQL =
        ...
        "    SUM(pd.nb_vivants) AS nb_vivants, " +
        "    SUM(pd.nb_coupes) AS nb_coupes, " +
        "    SUM(pd.nb_morts) AS nb_morts, " +
        ...
        "  a.nb_arbres, a.nb_echantillons, a.nb_vivants, a.nb_coupes, a.nb_morts, " +
        "  a.densite_ha, a.surface_terriere_ha, a.volume_ha, " +
    ...
    public ResponseEntity<Map<String, Object>> getFormationDetail(@PathVariable String formation) {
        ...
        result.put("profondeur_sol", jdbc.queryForList(
            "WITH " + LOC_CTE + " SELECT profondeur_sol AS code, COUNT(*) AS nb " +
            "FROM loc WHERE formation = ? AND profondeur_sol IS NOT NULL GROUP BY profondeur_sol ORDER BY profondeur_sol", formation));
        result.put("humus", jdbc.queryForList(
            "WITH " + LOC_CTE + " SELECT humus AS code, COUNT(*) AS nb " +
            "FROM loc WHERE formation = ? AND humus IS NOT NULL GROUP BY humus ORDER BY humus", formation));
        result.put("affleurement_rocheux", jdbc.queryForList(
            "WITH " + LOC_CTE + " SELECT affleurement_rocheux AS code, COUNT(*) AS nb " +
            "FROM loc WHERE formation = ? AND affleurement_rocheux IS NOT NULL GROUP BY affleurement_rocheux ORDER BY affleurement_rocheux", formation));
        result.put("cailloux", jdbc.queryForList(
            "WITH " + LOC_CTE + " SELECT cailloux AS code, COUNT(*) AS nb " +
            "FROM loc WHERE formation = ? AND cailloux IS NOT NULL GROUP BY cailloux ORDER BY cailloux", formation));
        ...
        // ── Structure — histogrammes circonférence C1,3 (20 cm), circonférence C0 (20 cm)
        // et hauteur (2 m) — sur les arbres vivants des placettes de cet écosystème.
        String treeCte =
            "WITH " + plotsCte + ", live AS ( " +
            "  SELECT t.tree_c1_3, t.tree_c0, t.tree_ht FROM tree t " +
        ...
        result.put("structure_c0", jdbc.queryForList(
            treeCte +
            "SELECT (FLOOR(tree_c0/20)*20)::int AS bin_start, COUNT(*) AS nb " +
            "FROM live WHERE tree_c0 IS NOT NULL GROUP BY bin_start ORDER BY bin_start", formation));
