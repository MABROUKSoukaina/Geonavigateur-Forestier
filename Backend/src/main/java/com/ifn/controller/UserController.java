...
public class UserController {
    ...
    record UserView(Long id, String username, String fullName, String role, String team, boolean enabled) {
        static UserView of(AppUser u) {
            return new UserView(u.getId(), u.getUsername(), u.getFullName(), u.getRole(), u.getTeam(), u.isEnabled());
        }
    }

    record CreateRequest(String username, String password, String fullName, String role, String team) {}
    record UpdateRequest(String fullName, String role, String team, Boolean enabled) {}
    ...
    public ResponseEntity<?> create(@RequestBody CreateRequest req) {
        try {
            AppUser u = service.create(req.username(), req.password(), req.fullName(), req.role(), req.team());
    ...
    public ResponseEntity<?> update(@PathVariable Long id, @RequestBody UpdateRequest req) {
        try {
            AppUser u = service.update(id, req.fullName(), req.role(), req.team(), req.enabled());
            return ResponseEntity.ok(UserView.of(u));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
        }
