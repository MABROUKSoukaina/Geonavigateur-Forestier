...
function inverse3(p) {
  ...
  if (!this.es) {
    ...
    if (g2 === 0 && h2 === 0) {
      ...
      lon = adjust_lon_default(Math.atan2(g2, h2) + this.long0, this.over);
    }
  } else {
    ...
    if (Math.abs(phi) < HALF_PI) {
      ...
      lon = adjust_lon_default(this.long0 + d2 * (1 - ds / 6 * (1 + 2 * t + c - ds / 20 * (5 + 28 * t + 24 * ts + 8 * c * t + 6 * c - ds / 42 * (61 + 662 * t + 1320 * ts + 720 * ts * t)))) / cos_phi, this.over);
    } else {
...
var parseZip = async function(buffer, whiteList) {
  ...
  for (key in zip) {
    ...
    if (key.slice(-4).toLowerCase() === ".shp") {
      ...
      names37.push(key.slice(0, -3) + key.slice(-3).toLowerCase());
  ...
  const geojson = names37.map(function(name) {
    let parsed, dbf;
